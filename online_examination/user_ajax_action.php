	...
	if($_POST["page"] == 'enroll_exam')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			foreach($result as $row)
			{
				...
				if($row["online_exam_status"] == 'Started')
				{
					$view_exam = '<a href="view_exam.php?code='.$row["online_exam_code"].'" class="btn btn-info btn-sm">Take Exam Test</a>';
				}
				if($row["online_exam_status"] == 'Completed')
				{
					$view_exam = '<a href="view_exam.php?code='.$row["online_exam_code"].'" class="btn btn-info btn-sm">View Exam Result</a>';
				}
		...
		if($_POST['action'] == 'load_question')
		{
			...
			foreach($result as $row)
			{
				...
				foreach($sub_result as $sub_row)
				{
					...
					$user_answer_option_result = $exam->query_result();

					$checked = '';

					foreach($user_answer_option_result as $user_answer_option)
					{
						$option_number = $user_answer_option['user_answer_option'];
					...
					if($count == 1 ){
						...
						<div class="col-md-6" style="margin-bottom:32px;">
							<div class="radio">
								<label><h4><input type="radio" checked name="option_1" class="answer_option" data-question_id="'.$row["question_id"].'" data-id="'.$count.'"/>&nbsp;'.$sub_row["option_title"].'</h4></label>
							</div>
						</div>
						';
					}else{
						...
						<div class="col-md-6" style="margin-bottom:32px;">
							<div class="radio">
								<label><h4><input type="radio" name="option_1" class="answer_option" data-question_id="'.$row["question_id"].'" data-id="'.$count.'"/>&nbsp;'.$sub_row["option_title"].'</h4></label>
							</div>
						</div>
						';
						}
